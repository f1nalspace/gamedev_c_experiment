...
external void Win32OpenGLRender(RenderState *renderState) {
	...
	for (U32 commandIndex = 0; commandIndex < renderState->commandCount; ++commandIndex) {
		...
		// TODO: This is not fast by any means
		Mat4f scaleMat = MakeScale4(V3(command->transform.scale.x, command->transform.scale.y, 0.0f));
		Mat4f translationMat = MakeTranslation4(command->transform.pos);
		Mat4f rotationMat = MakeRotation4(command->transform.rot);
		...
		switch (command->type) {
			...
			{
				LinesRenderCommand *lines = &command->lines;
